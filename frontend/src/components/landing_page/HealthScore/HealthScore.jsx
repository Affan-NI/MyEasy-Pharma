...
const App = () => {
  ...
  const calculateScore = () => {
    ...
    const overall = 0.35 * gss + 0.25 * lri + 0.2 * ssgs + 0.2 * ecrs;
    ...
    <div className="min-h-screen bg-white font-sans text-slate-900 pb-20">
      ...
      <div className="max-w-6xl mx-auto px-6 -mt-10 relative z-20">
        ...
        {results && (
          <div
            ref={resultRef}
            className="mt-8 pt-6 border-t border-slate-100 animate-in fade-in slide-in-from-bottom-5 duration-500"
          >
            <div className="bg-white rounded-3xl p-4 md:p-6 shadow-xl border border-slate-100 text-center max-w-2xl mx-auto">
              <p className="text-slate-400 font-bold uppercase tracking-widest text-[12px] mb-0.5">
                Overall Health Score
              </p>
              <div
                className={`text-5xl font-black leading-none mb-1 ${getStatusColor(results.overall)}`}
              >
                {results.overall}
              </div>
              <div
                className={`text-xg font-bold uppercase tracking-[0.2em] mb-4 ${getStatusColor(results.overall)}`}
              >
                {results.overall >= 80
                  ? "Healthy"
                  : results.overall >= 60
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left border-t border-slate-100 pt-5">
                <div>
                  <h4 className="font-bold text-slate-900 mb-2 flex items-center gap-1.5 text-lg">
                    <AlertTriangle className="text-orange-500" size={20} /> Top
                    Risk Drivers
                  </h4>
                  <div className="space-y-1.5">
                    {results.topRisks.map((risk, i) => (
                      <div
                        key={i}
                        className="bg-white p-2 rounded-lg border border-slate-100 border-l-2 border-l-[#f06529] flex justify-between items-center shadow-sm"
                      >
...
const InputField = ({ id, label, hint, value, onChange }) => (
  <div className="space-y-2 group">
    <div className="flex items-center gap-1.5">
      <label
        htmlFor={id}
        className="block text-[12px] font-black text-slate-500 uppercase tracking-widest"
      >
        {label}
      </label>
    ...
    />
    <p className="text-[13px] text-slate-500 font-semibold italic mt-1.5 leading-relaxed">
      {hint}
    </p>
  </div>
);
...
const ScorePill = ({ label, value }) => (
  <div className="bg-white border border-slate-100 rounded-xl p-2 shadow-sm text-center">
    <div className="text-xg font-black text-emerald-800 leading-none mb-0.5">
      {value}
    </div>
