...
import { MapPin, Search, Home, Briefcase, Plus, Navigation, Check } from 'lucide-react';
...
const Addres = () => {
  ...
  const [savedAddresses] = useState([
    { id: 1, type: 'Home', address: '123 Maple Street, Springfield', icon: <Home size={16} /> },
    { id: 2, type: 'Office', address: '888 Tech Plaza, Downtown', icon: <Briefcase size={16} /> },
  ]);
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="p-4 border-b border-gray-50">
              ...
              {suggestions.length > 0 && (
                <div className="mt-2 space-y-1">
                  {suggestions.map((s, i) => (
                    <div key={i} className="flex items-center gap-2 p-2 hover:bg-emerald-50 rounded-md cursor-pointer text-xs text-gray-600 group">
                      <MapPin size={12} className="text-gray-400 group-hover:text-emerald-500" />
                      <span>{s}</span>
                    </div>
                  ))}
                </div>
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mt-2">Saved Addresses</div>
                ...
                {savedAddresses.map((addr) => (
                  <div 
                    ...
                    className={`flex items-start gap-3 p-2.5 rounded-xl cursor-pointer transition-all border ${
                      selectedType === addr.type ? 'bg-emerald-50 border-emerald-100' : 'bg-white border-transparent hover:bg-gray-50'
                    }`}
                  >
                    <div className={`p-2 rounded-lg ${selectedType === addr.type ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-500'}`}>
                      {addr.icon}
                    </div>
                    <div className="flex-1">
                      <div className="flex justify-between items-center">
                        <span className="text-xs font-bold text-gray-800">{addr.type}</span>
                        {selectedType === addr.type && <Check size={12} className="text-emerald-600" />}
                      </div>
                      <p className="text-[11px] text-gray-500 truncate w-40">{addr.address}</p>
                    </div>
                  </div>
                ))}
